...
final class MoviesRepository {
    ...
    // MARK: - Initialization
    init(
        service: UpcomingMoviesAPIServiceProtocol = UpcomingMoviesAPIService(),
    ...
    func fetchGenres(
        ...
        genresRepository.genres { result in
            ...
            case let .success(genresSearch):
                self.handle(movies: movies, configuration: configuration, genresSearch: genresSearch, completion: completion)
